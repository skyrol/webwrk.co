...
<p class="order-info"><?php
	echo wp_kses_post( apply_filters( 'woocommerce_order_tracking_status', sprintf(
		esc_html__( 'Order #%1$s was placed on %2$s and is currently %3$s.', 'norebro' ),
...
<?php if ( $notes = $order->get_customer_order_notes() ) : ?>
	<h2 class="second-title"><?php esc_html_e( 'Order Updates', 'norebro' ); ?></h2>
	<ol class="commentlist notes">
		<?php foreach ( $notes as $note ) : ?>
		<li class="comment note">
			<div class="comment_container">
				<div class="comment-text">
					<p class="meta"><?php echo date_i18n( 'l jS \o\f F Y, h:ia', strtotime( $note->comment_date ) ); ?></p>
