	...
	<table class="shop_table cart wishlist_table" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo ( is_user_logged_in() ) ? esc_attr( $wishlist_meta['ID'] ) : '' ?>" data-token="<?php echo ( ! empty( $wishlist_meta['wishlist_token'] ) && is_user_logged_in() ) ? esc_attr( $wishlist_meta['wishlist_token'] ) : '' ?>">
		...
		<tr>
			...
			<th class="product-name">
				<span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'norebro'  ) ) ?></span>
			</th>
			...
			<?php if( $show_price ) : ?>
				...
				<th class="product-price">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'norebro'  ) ) ?>
					</span>
				</th>
			...
			<?php if( $show_stock_status ) : ?>
				...
				<th class="product-stock-stauts">
					<span class="nobr">
						<?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'norebro'  ) ) ?>
					</span>
				</th>
		...
		if( count( $wishlist_items ) > 0 ) :
					...
					<tr id="yith-wcwl-row-<?php echo esc_attr( $item['prod_id'] ); ?>" data-row-id="<?php echo esc_attr( $item['prod_id'] ); ?>">
						<?php if( $show_cb ) : ?>
							<td class="product-checkbox">
								<input type="checkbox" value="<?php echo esc_attr( $item['prod_id'] ) ?>" name="add_to_cart[]" <?php echo esc_attr( $product->product_type != 'simple' ) ? 'disabled="disabled"' : '' ?>/>
							</td>
						<?php endif ?>

						<td class="product-thumbnail">
							<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">
                               <?php vprintf('%s', $product->get_image()); ?>
							</a>
						</td>

						<td class="product-name">
							<h4 class="title text-left">
								<a href="<?php echo esc_url( get_permalink( apply_filters( 'woocommerce_in_cart_product', $item['prod_id'] ) ) ) ?>">	<?php echo apply_filters( 'woocommerce_in_cartproduct_obj_title', get_post( $product->get_id() )->post_title, $product ) ?>
								</a>
						...
						<?php if( $show_price ) : ?>
							<td class="product-price">
								...
								elseif( $product->price != '0' ) {
								    vprintf('%s', $product->get_price_html());
								}
								else {
									echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'norebro'  ) );
								}
								?>
							</td>
						<?php endif ?>
						...
						<?php if( $show_stock_status ) : ?>
							<td class="product-stock-status">
								...
								if( $stock_status == 'out-of-stock' ) {
									$stock_status = "Out";
									echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'norebro'  ) . '</span>';
								} else {
									$stock_status = "In";
									echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'norebro'  ) . '</span>';
								}
								?>
							</td>
						<?php endif ?>
						...
						<td class="product-add-to-cart">
							...
							if( $show_dateadded && isset( $item['dateadded'] ) ):
								echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on : %s', 'norebro'  ), date_i18n( get_option( 'date_format' ), strtotime( $item['dateadded'] ) ) ) . '</span>';
							endif;
							?>
							...
							<select class="change-wishlist selectBox">
								<option value=""><?php esc_html_e( 'Move', 'norebro'  ) ?></option>
								<?php
								...
								?>
									<option value="<?php echo esc_attr( $wl['wishlist_token'] ) ?>">
										<?php
										$wl_title = ! empty( $wl['wishlist_name'] ) ? $wl['wishlist_name'] : $default_wishlsit_title;
										if( $wl['wishlist_privacy'] == 1 ) {
											$wl_privacy = esc_html__( 'Shared', 'norebro'  );
										}
										elseif ( $wl['wishlist_privacy'] == 2 ) {
											$wl_privacy = esc_html__( 'Private', 'norebro'  );
										}
										else {
											$wl_privacy = esc_html__( 'Public', 'norebro'  );
										}
							...
							<?php if( $is_user_owner && $repeat_remove_button ): ?>
								<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove_from_wishlist button" title="<?php esc_html_e( 'Remove this product', 'norebro'  ) ?>"><?php esc_html_e( 'Remove', 'norebro'  ) ?></a>
							<?php endif; ?>
						</td>

						<?php if( $is_user_owner ): ?>
						<td class="product-remove">
							<a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="remove-link" title="<?php esc_attr_e( 'Remove this product', 'norebro'  ) ?>"><i class="ion-ios-close-outline"></i></a>
						</td>
						<?php endif; ?>
			...
			<tr>
				<td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php esc_html_e( 'No products were added to the wishlist', 'norebro'  ) ?></td>
			</tr>
		<?php
		endif;
		...
		if( ! empty( $page_links ) ) : ?>
			<tr class="pagination-row">
				<td colspan="<?php echo esc_attr( $column_count ) ?>"><?php echo wp_kses( $page_links, 'default' ); ?></td>
			</tr>
		<?php endif ?>
		</tbody>
				...
				<td colspan="<?php echo esc_attr( $column_count ) ?>">
					<?php if( $show_cb ) : ?>
						<div class="custom-add-to-cart-button-cotaniner">
							<a href="<?php echo esc_url( add_query_arg( array( 'wishlist_products_to_add_to_cart' => '', 'wishlist_token' => $wishlist_meta['wishlist_token'] ) ) ) ?>" class="button alt" id="custom_add_to_cart"><?php echo apply_filters( 'yith_wcwl_custom_add_to_cart_text', esc_html__( 'Add the selected products to the cart', 'norebro'  ) ) ?></a>
						</div>
					<?php endif; ?>
					...
					<?php if ( is_user_logged_in() && $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
						<div class="ask-an-estimate-button-container">
							...
							<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_text', esc_html__( 'Ask for an estimate', 'norebro'  ) ) ?>
						</a>
						</div>
					<?php endif; ?>
	...
	<div id="ask_an_estimate_popup">
		<form action="<?php echo esc_url( $ask_estimate_url ); ?>" method="post" class="wishlist-ask-an-estimate-popup">
			<?php if( ! empty( $additional_info_label ) ):?>
				<label for="additional_notes"><?php echo wp_kses( $additional_info_label, 'default' ); ?></label>
			<?php endif; ?>
			<textarea id="additional_notes" name="additional_notes"></textarea>

			<button class="btn button ask-an-estimate-button ask-an-estimate-button-popup" >
				<?php echo apply_filters( 'yith_wcwl_ask_an_estimate_icon', '<i class="fa fa-shopping-cart"></i>' )?>
				<?php esc_html_e( 'Ask for an estimate', 'norebro'  ) ?>
			</button>
		</form>
	</div>
<?php endif; ?>