...
if ( $available_gateways = WC()->payment_gateways->get_available_payment_gateways() ) : ?>
	<form id="add_payment_method" method="post">123123
		<div id="payment" class="woocommerce-Payment">
				...
				<?php
					...
					foreach ( $available_gateways as $gateway ) {
						...
						<li class="woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr( $gateway->id ); ?> payment_method_<?php echo esc_attr( $gateway->id ); ?>">
							...
							<label for="payment_method_<?php echo esc_attr( $gateway->id ); ?>"><?php echo esc_attr( $gateway->get_title() ); ?> <?php vprintf('%s', $gateway->get_icon()); ?></label>
							<?php
			...
			<div class="form-row">
				...
				<input type="submit" class="woocommerce-Button woocommerce-Button--alt btn alt" id="place_order" value="<?php esc_attr_e( 'Add Payment Method', 'norebro' ); ?>" />
				<input type="hidden" name="woocommerce_add_payment_method" id="woocommerce_add_payment_method" value="1" />
			</div>
		</div>
	</form>
<?php else : ?>
	<p><?php esc_html_e( 'Sorry, it seems that there are no payment methods which support adding a new payment method. Please contact us if you require assistance or wish to make alternate arrangements.', 'norebro' ); ?></p>
<?php endif; ?>
